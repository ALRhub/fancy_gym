...
def plot_trajs_j(position, velocity, start_index=0, end_index=100, plot_sampling=True, plot_constrs=True, dof=3):
    if plot_sampling:
        dt = 0.001
    else:
        dt = 0.02
...
def plot_trajs_c(position, velocity, start_index=0, end_index=100):
    table_width = 1.038
    table_length = 1.948
    ...
    plt.plot(position[:, 0] - 1.51, position[:, 1])
